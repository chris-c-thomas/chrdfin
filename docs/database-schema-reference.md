...
CREATE TABLE IF NOT EXISTS assets (
    ...
    source      VARCHAR DEFAULT 'massive',-- provider that supplied this row (Phase 1C)
...
CREATE TABLE IF NOT EXISTS daily_prices (
    ...
    volume    BIGINT,
    source    VARCHAR DEFAULT 'massive', -- provider provenance (Phase 1C)
...
CREATE TABLE IF NOT EXISTS splits (
    ...
    split_from      DOUBLE NOT NULL,     -- pre-split share count factor
    split_to        DOUBLE NOT NULL,     -- post-split share count factor
    adjustment_type VARCHAR,             -- 'forward_split' | 'reverse_split' | 'stock_dividend' | 'unknown'
    source          VARCHAR DEFAULT 'massive',
    PRIMARY KEY (ticker, execution_date)
);
```

> Backtest math should multiply historical share counts by `split_to / split_from`. A 4-for-1 split is `split_from=1, split_to=4` — historical shares × 4.

### Table: `macro_series`

```sql
CREATE TABLE IF NOT EXISTS macro_series (
    series_id VARCHAR(50) NOT NULL,      -- closed enum: 'treasury_3_mo', 'treasury_10_y', 'cpi_yoy', 'unemployment_rate'
    date      DATE NOT NULL,
...
The `source` column on `assets`, `daily_prices`, `dividends`, `splits`, and `macro_series` lets multiple providers coexist without clobbering each other. Storage upserts compare priority via a SQL `CASE` expression generated by `storage::source::priority_case_sql`; a higher-priority row never gets overwritten by a lower-priority one. Current priorities:
...
See `docs/sync-architecture.md` for the full multi-provider story.
